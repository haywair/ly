...
class Admin extends Backend
{
    ...
    public function index()
    {
        if ($this->request->isAjax())
        {
            ...
            /*$total = $this->model
                    ...
                    ->fetchSql(true)
                    ->count();*/
            $total = $this->model->where('id','in',$this->childrenAdminIds)->count();
    ...
    public function del($ids = "")
    {
        if ($ids)
        {
            ...
            $adminAll = $this->model->where('id', 'in', $ids)->select();
            $uids = model('AuthGroupAccess')->where('group_id', 'in', $childrenGroupIds)->column('uid');
            $adminList = [];
            foreach($adminAll as $ka_=> $va){
                if(in_array($va['id'],array_keys($uids))){
                    $adminList[] = $va;
                }
            }
