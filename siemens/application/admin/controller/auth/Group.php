...
class Group extends Backend
{
     ...
     * 设置前台权限
     */
    public function front($ids = NULL)
    {
        $row = $this->model->get(['id' => $ids]);
        if (!$row)
            $this->error(__('No Results were found'));
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                $data = ['pid'=>$params['pid'],'front_rules'=>$params['rules']];
                $row->save($data);
                $this->success();
            }
            $this->error();
    ...
    public function frontroletree()
    {
        ...
        {
            $id = $id ? $id : NULL;
            $ruleList = collection(model('AuthFrontrule')->order('weigh', 'desc')->select())->toArray();
            //读取父类角色所有节点列表
            $parentRuleList = [];
            if (in_array('*', explode(',', $parentgroupmodel->front_rules)))
            {
                $parentRuleList = $ruleList;
            }
            else
            {
                $parent_rule_ids = explode(',', $parentgroupmodel->front_rules);
                foreach ($ruleList as $k => $v)
                {
                    if (in_array($v['id'], $parent_rule_ids))
                    {
           ...
           /* //读取当前角色下规则ID集合*/
            ...
            $superadmin = $this->auth->isSuperAdmin();
            //当前拥有的规则ID集合
            $current_rule_ids = $id ? explode(',', $currentgroupmodel->front_rules) : [];
