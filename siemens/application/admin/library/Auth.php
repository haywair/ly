...
class Auth extends \fast\Auth
{
    ...
    public function getFrontGroups($uid = null)
    {
        $uid = is_null($uid) ? $this->id : $uid;
        return parent::getGroupsFront($uid);
    }
     ...
     * @return array
     */
    public function getFrontChildrenAdminIds($withself = false)
    {
        $groupIds = $this->getChildrenGroupIds(false);
        $childrenAdminIds = [];
        $authGroupList = model('FrontAuthGroupAccess')
            ->field('uid,group_id')
            ->where('group_id', 'in', $groupIds)
            ->select();
