...
class Auth
{
    ...
    protected $config = [
        ...
        'front_auth_group_access'   =>  'front_auth_group_access'
    ];
     ...
     *              ...)
     */
    public function getGroupsFront($uid)
    {
        static $groups = [];
        if (isset($groups[$uid]))
        {
        ...
        // 转换表名
        $auth_group_access = Loader::parseName($this->config['front_auth_group_access'], 1);
        $auth_group = Loader::parseName($this->config['auth_group'], 1);
        // 执行查询
        $user_groups = Db::view($auth_group_access, 'uid,group_id')
            ->view($auth_group, 'id,pid,name,rules', "{$auth_group_access}.group_id={$auth_group}.id", 'LEFT')
